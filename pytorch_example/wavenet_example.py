...
class ResidualStack(torch.nn.Module):
    ...
    def forward(self, x, skip_size):
        ...
        for res_block in self.res_blocks:
            ...
            # print len(skip_connections),len(skip_connections[0])
...
class  WaveNet(torch.nn.Module):
    ...
    def calc_receptive_fields(layer_size, stack_size):
        ...
        print [2 ** i for i in range(0, layer_size)]
    ...
    def forward(self, x):
        ...
        """
        # print ("x  start shape",x.shape)
        # print x.transpose(1,2)
        # output = x.transpose(1, 2)
        output = x
        # print ("first layer input shape",output.shape)

        output_size = self.calc_output_size(output)
        # print ("output_size",output_size)

        output = self.causal(output)
        # print ("after conv1d",output.shape)

        skip_connections = self.res_stack(output, output_size)
        # print ("after skip_connections",skip_connections.shape)
        # print ("after skip_connections",skip_connections)

        output = torch.sum(skip_connections, dim=0)
        # print ("skip_connections",output.shape)
        output = self.densnet(output)
        # print ("densnet",output.shape)
